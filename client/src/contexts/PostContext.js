...
import {apiURL, POSTS_LOADED_FAIL, POSTS_LOADED_SUCCESS, ADD_POST, UPDATE_POST, DELETE_POST, FIND_POST} from './constants'
...
const PostContextProvider = ({children}) => {
  ...
  const getPosts = async () => {
    try {
      ...
      } else dispatch({type: POSTS_LOADED_FAIL})
    } catch (error) {
        dispatch({type: POSTS_LOADED_FAIL})
    }
  }

  // Find post is being updated
  ...
  const addPost = async newPost => {
    try {
      ...
      return error.response.data? error.response.message : {success: false, message: 'Server error'}
    }
  }
